...
import { ContentButton } from "../components.js"

import './SaveHandler.css'

function SaveHandler({ onLoad, downloadData }) {
  ...
  return (
    <section className="save-handler">
      <label className="save-handler-label">
        Load a save file (save_&lt;slot&gt;.es3):
        <input type="file" id="fileInput" accept=".es3" onChange={handleSaveChange} className="save-handler-input"/>
      </label>
      {downloadData &&
        <ContentButton onClickFunc={DownloadSave} label="Download Modified Save"/>
      }
    </section>
  )
}
